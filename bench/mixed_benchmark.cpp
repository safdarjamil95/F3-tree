...
int main(int argc, char** argv) {
  try {
    ...
    if (options.key_count < 2) {
      throw std::runtime_error("mixed benchmark requires at least two keys");
    }
    ...
    const std::size_t warmup_count = keys.size() / 2;
    for (std::size_t i = 0; i < warmup_count; ++i) {
      tree.warmup_insert(keys[i]);
    }

    std::vector<std::future<void> > workers;
    const std::size_t active_count = keys.size() - warmup_count;
    const std::size_t chunk = active_count / options.producer_threads;

    const std::chrono::steady_clock::time_point start =
        std::chrono::steady_clock::now();
    for (std::size_t tid = 0; tid < options.producer_threads; ++tid) {
      const std::size_t from = warmup_count + tid * chunk;
      const std::size_t to =
          (tid + 1 == options.producer_threads) ? keys.size() : from + chunk;
      workers.push_back(std::async(std::launch::async,
                                   [&tree, &keys, warmup_count, tid](
                                       std::size_t begin, std::size_t end) {
                                     for (std::size_t i = begin; i < end; ++i) {
                                       const std::size_t search_index =
                                           (i - warmup_count) % warmup_count;
                                       tree.buffered_insert(keys[i], tid);
                                       static_cast<void>(
                                           tree.global_contains(keys[search_index]));
                                     }
                                   },
                                   from, to));
    }
    ...
    const std::chrono::steady_clock::time_point op_stop =
        std::chrono::steady_clock::now();
    ...
    const std::chrono::steady_clock::time_point stop =
        std::chrono::steady_clock::now();
    const std::chrono::microseconds op_elapsed =
        std::chrono::duration_cast<std::chrono::microseconds>(op_stop - start);
    const std::chrono::microseconds total_elapsed =
        std::chrono::duration_cast<std::chrono::microseconds>(stop - start);
    const f3tree::FutureBTreeStats stats = tree.stats();

    std::cout << "mixed_mode=" << options.evaluator_mode << '\n';
    std::cout << "mixed_placement=" << options.placement_mode << '\n';
    std::cout << "mixed_ops_us=" << op_elapsed.count() << '\n';
    std::cout << "mixed_total_us=" << total_elapsed.count() << '\n';
    std::cout << "mixed_checkpoint_count=" << stats.checkpoint_count << '\n';
    std::cout << "mixed_async_checkpoint_count=" << stats.async_checkpoint_count << '\n';
    std::cout << "mixed_checkpoint_time_us=" << stats.checkpoint_time_us << '\n';
    std::cout << "mixed_replayed_ops=" << stats.replayed_ops << '\n';
    std::cout << "mixed_replayed_nodes=" << stats.replayed_nodes << '\n';
    std::cout << "mixed_topology_nodes=" << stats.placement_topology_nodes << '\n';
    std::cout << "mixed_bound_evaluators=" << stats.bound_evaluator_threads << '\n';
    std::cout << "mixed_bound_producers=" << stats.bound_producer_threads << '\n';
    std::cout << "mixed_hard_numa_bound=" << stats.hard_numa_memory_bound << '\n';
    if (total_elapsed.count() > 0) {
      std::cout << "mixed_ops_per_sec="
                << (active_count * 1000000.0 / static_cast<double>(total_elapsed.count())) << '\n';
    }
    std::cout << "mixed_found=" << found << "/" << keys.size() << '\n';
    return 0;
  } catch (const std::exception& ex) {
    std::cerr << "mixed_benchmark error: " << ex.what() << '\n';
    return 1;
  }
}
